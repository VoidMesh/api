...
func TestService_HarvestResourceNode(t *testing.T) {
	cleanup := testutil.SetupTest(t, testutil.DefaultTestConfig())
	defer cleanup()
	...
	tests := []struct {
		...
		{
			name:           "successful harvest with primary yield only",
			characterID:    "550e8400e29b41d4a716446655440000",
			resourceNodeID: 1,
			setupMocks: func(mockDB *MockDatabaseInterface, mockResourceService *MockResourceNodeServiceInterface, mockLogger *MockLoggerInterface) {
				resourceNode := createTestResourceNode(1, 1)
				mockDB.On("GetResourceNode", mock.Anything, int32(1)).Return(resourceNode, nil)
				...
				// Mock for AddInventoryItem call
				expectedUUID := createTestCharacterUUID("550e8400e29b41d4a716446655440000")
				mockDB.On("InventoryItemExists", mock.Anything, mock.MatchedBy(func(params db.InventoryItemExistsParams) bool {
					return params.CharacterID == expectedUUID && params.ResourceNodeTypeID == 1
				})).Return(false, nil)
				
				createdItem := createTestInventoryItem(1, "550e8400e29b41d4a716446655440000", 1, 3) // Use a fixed value for the test
				mockDB.On("CreateInventoryItem", mock.Anything, mock.MatchedBy(func(params db.CreateInventoryItemParams) bool {
					return params.CharacterID == expectedUUID && params.ResourceNodeTypeID == 1 && params.Quantity >= 1 && params.Quantity <= 5
				})).Return(createdItem, nil)
				
				// Since we can't control randomness in the test, we'll return a known resource type
				mockResourceService.On("GetResourceNodeTypes", mock.Anything).Return(resourceTypes, nil)
				
				mockLogger.On("Debug", "Harvesting resource node", "character_id", "550e8400e29b41d4a716446655440000", "resource_node_id", int32(1))
				mockLogger.On("Debug", "Adding inventory item", "character_id", "550e8400e29b41d4a716446655440000", "resource_type", resourceNodeV1.ResourceNodeTypeId(1), "quantity", mock.AnythingOfType("int32"))
				mockLogger.On("Debug", "Added inventory item", "character_id", "550e8400e29b41d4a716446655440000", "resource_type", resourceNodeV1.ResourceNodeTypeId(1), "new_quantity", mock.AnythingOfType("int32"))
				mockLogger.On("Debug", "Completed resource harvest", 
					"character_id", "550e8400e29b41d4a716446655440000",
					"resource_node_id", int32(1),
					"primary_yield", mock.AnythingOfType("int32"),
					"total_results", mock.AnythingOfType("int"))
			},
			expectError:   false,
			expectResults: 1, // At minimum, primary yield
			expectPrimary: true,
		},
		{
			name:           "resource node not found",
			characterID:    "550e8400e29b41d4a716446655440000",
			resourceNodeID: 999,
			setupMocks: func(mockDB *MockDatabaseInterface, mockResourceService *MockResourceNodeServiceInterface, mockLogger *MockLoggerInterface) {
				mockDB.On("GetResourceNode", mock.Anything, int32(999)).Return(db.ResourceNode{}, sql.ErrNoRows)
				
				mockLogger.On("Debug", "Harvesting resource node", "character_id", "550e8400e29b41d4a716446655440000", "resource_node_id", int32(999))
				mockLogger.On("Error", "Failed to get resource node", "resource_node_id", int32(999), "error", sql.ErrNoRows)
			},
			expectError:    true,
			expectCode:     codes.NotFound,
			expectErrorMsg: "resource node not found",
		},
		{
			name:           "resource node types not found",
			characterID:    "550e8400e29b41d4a716446655440000",
			resourceNodeID: 1,
			setupMocks: func(mockDB *MockDatabaseInterface, mockResourceService *MockResourceNodeServiceInterface, mockLogger *MockLoggerInterface) {
				resourceNode := createTestResourceNode(1, 1)
				...
				mockLogger.On("Debug", "Harvesting resource node", "character_id", "550e8400e29b41d4a716446655440000", "resource_node_id", int32(1))
				mockLogger.On("Error", "Failed to get resource node types", "error", sql.ErrConnDone)
			},
			expectError:    true,
			expectCode:     codes.Internal,
			expectErrorMsg: "failed to get resource information",
		},
		{
			name:           "resource node type not found in types list",
			characterID:    "550e8400e29b41d4a716446655440000",
			resourceNodeID: 1,
			setupMocks: func(mockDB *MockDatabaseInterface, mockResourceService *MockResourceNodeServiceInterface, mockLogger *MockLoggerInterface) {
				resourceNode := createTestResourceNode(1, 999) // Type ID that doesn't exist
				mockDB.On("GetResourceNode", mock.Anything, int32(1)).Return(resourceNode, nil)
				
				resourceTypes := []*resourceNodeV1.ResourceNodeType{
					createTestResourceNodeType(1, "Herb"), // Different ID
				}
				mockResourceService.On("GetResourceNodeTypes", mock.Anything).Return(resourceTypes, nil)
			...
			expectError:    true,
			expectCode:     codes.Internal,
			expectErrorMsg: "resource node type not found",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := testutil.CreateTestContext()
			results, updatedItem, err := service.HarvestResourceNode(ctx, tt.characterID, tt.resourceNodeID)
			...
			if tt.expectError {
				...
				assert.NoError(t, err)
				assert.GreaterOrEqual(t, len(results), tt.expectResults)
				
				if tt.expectPrimary {
					assert.NotNil(t, updatedItem)
					// Check that at least one result is primary (not secondary drop)
					foundPrimary := false
					for _, result := range results {
						if !result.IsSecondaryDrop {
							foundPrimary = true
							assert.True(t, result.Quantity >= 1 && result.Quantity <= 5, "Primary yield should be within expected range")
							break
						}
					}
					assert.True(t, foundPrimary, "Should have at least one primary harvest result")
				}
			}
